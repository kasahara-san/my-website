...
//===============================================================
$(function () {
    const journalCount = $('#journal-papers').next('ul').children('li').length;
    const conferenceCount = $('#international-conferences').next('ul').children('li').length;
    const oralCount = $('#oral-presentations').next('ul').children('li').length;
    const awardsCount = $('#awards').next('ul').children('li').length;
  
    const updateCount = (selector, count) => {
      const link = $(`a[href="${selector}"]`);
      const text = link.text().replace(/\s*\(\d+\)\s*$/, ''); // 既存の()を削除
      link.text(`${text} (${count})`);
    };
  
    updateCount('#journal-papers', journalCount);
    updateCount('#international-conferences', conferenceCount);
    updateCount('#oral-presentations', oralCount);
    updateCount('#awards', awardsCount);
  });
